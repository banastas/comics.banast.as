...
export const SlabbedComicsDetail: React.FC<SlabbedComicsDetailProps> = ({ 
  ...
  const totalComics = slabbedComics.length;
  const totalValue = slabbedComics.reduce((sum, comic) => sum + comic.purchasePrice, 0);
  const currentValue = slabbedComics.reduce((sum, comic) => sum + (comic.currentValue || comic.purchasePrice), 0);
  const gainLoss = currentValue - totalValue;
  const gainLossPercentage = totalValue > 0 ? (gainLoss / totalValue) * 100 : 0;
  const averageGrade = totalComics > 0 ? slabbedComics.reduce((sum, comic) => sum + comic.grade, 0) / totalComics : 0;
  const signedCount = slabbedComics.filter(comic => comic.signedBy.trim() !== '').length;
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-300">
                    {totalComics} slabbed comic{totalComics !== 1 ? 's' : ''} in collection
